...
def _conda_activate_handler(env_name_or_prefix):
    ...
    __xonsh__.execer.exec($($MAMBA_EXE shell activate -s xonsh -p @(env_name_or_prefix)),
                          glbs=__xonsh__.ctx,
                          filename="$(conda shell activate -s xonsh -p" + env_name_or_prefix + ")")


def _conda_deactivate_handler():
    __xonsh__.execer.exec($($MAMBA_EXE shell deactivate -s xonsh),
                          glbs=__xonsh__.ctx,
                          filename="$(conda shell deactivate -s xonsh)")
...
#     import os as _os
#     import sys as _sys
#     # _sys.path.insert(0, _os.path.join(_os.path.dirname(_os.path.dirname($MAMBA_EXE)), "condabin"))
#     del _os, _sys
