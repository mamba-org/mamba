...
namespace solv
{
    ...
    void ObjSolvableView::set_defaulted_keys(const std::vector<std::string>& keys) const
    {
        if (keys.empty())
        {
            // Store empty string for empty list to distinguish from "not set"
            // Actually, we should unset to avoid confusion
            // But libsolv's solvable_unset behavior varies, so just set empty string
