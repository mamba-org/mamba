...
namespace mamba
{
    ...
    public:
        ...
        // Stores a provided Mirror IFF no other mirror is already registered with the same id for the
        // specified mirror name. Returns true if the mirror has been stored, false otherwise.
    ...
    template <class MirrorType, class... Args>
    auto mirror_map::create_unique_mirror(const std::string& mirror_name, Args&&... args) -> MirrorType&
    {
        ...
        auto iter = std::find_if(mirrors.begin(), mirrors.end(), [new_id](auto& mirror) { return new_id == mirror->id(); });
        if (iter != mirrors.end())
