...
namespace mamba
{
    namespace
    {
        ...
        void create_mirrors(const specs::Channel& channel, mirror_map& mirrors)
        {
            if (!mirrors.has_mirrors(channel.id()))
            {
                for (const specs::CondaURL& url: channel.mirror_urls())
                {
                    mirrors.add_unique_mirror(channel.id(), make_mirror(url.str(specs::CondaURL::Credentials::Show)));
                }
            }
        }
