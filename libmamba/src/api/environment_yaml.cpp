...
namespace mamba
{
    ...
    )
    {
        ...
        result.channels = extract_channels_from_packages(prefix_data, channel_context, ignore_channels);
        ...
        for (const auto& pkg : records)
        {
            std::string spec = package_to_spec_string(pkg, channel_context, no_builds, ignore_channels);
    ...
    void yaml_contents_to_stream(const detail::yaml_file_contents& contents, std::ostream& out)
    {
        ...
        if (!deps_node.IsNull() && deps_node.size() > 0)
        {
    ...
    )
    {
        ...
        // This matches conda's standard behavior where "pip" should be in dependencies
        // when pip dependencies are present
        if (has_pip_deps && !has_pip_in_file && !use_uv)
        {
