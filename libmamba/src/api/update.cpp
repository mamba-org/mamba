        ...
        ) -> solver::Request
        {
            ...
            else
            {
                ...
                if (remove_not_specified)
                {
                    ...
                    {
                        // Verify if any package is installed but not present in the target
                        // environment file
                        found = false;
                        for (const auto& sp : specs)
                        {
                            if (mamba::specs::MatchSpec::parse(sp)->name().contains(
                                    it.second.name().str()
                                ))
                            {
                                found = true;
                        ...
                        // Remove any installed package but not present in the target environment
                        // file
                        if (!found)
                        {
                ...
                for (const auto& raw_ms : specs)
                {
                    ...
                    // mamba::specs::MatchSpec::parse(raw_ms)->name().str())
                    //     != specs.end()) continue;
                    for (auto& it : hist_map)
                    {
                        // Verify if any package in the target environment file is already installed
                        if (mamba::specs::MatchSpec::parse(raw_ms)->name().contains(
                                it.second.name().str()
                            ))
                        {
                            // Update any package in the target environment file that is already
                            // installed
                    ...
                    // Install any package in the target environment file that is not installed yet
                    // iff flag --prune is present
                    if (!found && remove_not_specified)
                    {
