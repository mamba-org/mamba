                        ...
                        if (obj.type == MSGPACK_OBJECT_MAP)
                        {
                            for (std::uint32_t j = 0; j < obj.via.map.size; ++j)
                            {
                                ...
                                try
                                {
                                    if (key == "packages")
                                    {
                                        // Convert map of PackageRecordDict
                                        if (val_obj.type == MSGPACK_OBJECT_MAP)
                                        {
                                            for (std::uint32_t k = 0; k < val_obj.via.map.size; ++k)
                                            {
                                                try
                                                {
                                                    std::string pkg_filename = msgpack_object_to_string(
                                                        val_obj.via.map.ptr[k].key
                                                    );
                                                ...
                                                {
                                                    LOG_WARNING
                                                        << "Failed to parse package record in 'packages': "
                                                        << e.what();
                                                }
                                            }
                                        }
                                    ...
                                    {
                                        // Handle "packages.conda" key name (Python format)
                                        if (val_obj.type == MSGPACK_OBJECT_MAP)
                                        {
                                            for (std::uint32_t k = 0; k < val_obj.via.map.size; ++k)
                                            {
                                                ...
                                                {
                                                    LOG_WARNING
                                                        << "Failed to parse package record in 'packages.conda': "
                                                        << e.what();
                                                }
                                            }
                                        }
