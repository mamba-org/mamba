...
namespace mamba::solver::resolvo
{
    ...
    void Database::add_repo_from_repodata_json(
        ...
        [[maybe_unused]] bool verify_artifacts
    )
    {
        // BEWARE:
    ...
    Database::version_sets_in_union(::resolvo::VersionSetUnionId version_set_union_id)
    {
        if (auto it = version_set_unions.find(version_set_union_id); it != version_set_unions.end())
        {
            return it->second;
        }
        static ::resolvo::Vector<::resolvo::VersionSetId> empty{};
        return empty;
    }
