...
namespace mamba::specs
{
    ...
    TEST(version, parse)
    {
        ...
        {
            EXPECT_EQ(Version::parse(raw), expected);
        }
        ...
        // Lowercase and strip
        EXPECT_EQ(Version::parse("0.4.1.rc"), Version::parse("  0.4.1.RC  "));
        EXPECT_EQ(Version::parse("  0.4.1.RC  "), Version::parse("0.4.1.rc"));

        // Functional assertions
        EXPECT_EQ(Version::parse("  0.4.rc  "), Version::parse("0.4.RC"));
        EXPECT_EQ(Version::parse("0.4"), Version::parse("0.4.0"));
        EXPECT_NE(Version::parse("0.4"), Version::parse("0.4.1"));
        EXPECT_EQ(Version::parse("0.4.a1"), Version::parse("0.4.0a1"));
        EXPECT_NE(Version::parse("0.4.a1"), Version::parse("0.4.1a1"));
    }

    TEST(version, parse_invalid)
    {
        // Wrong epoch
        EXPECT_THROW(Version::parse("!1.1"), std::invalid_argument);
        EXPECT_THROW(Version::parse("-1!1.1"), std::invalid_argument);
        EXPECT_THROW(Version::parse("foo!1.1"), std::invalid_argument);
        EXPECT_THROW(Version::parse("0post1!1.1"), std::invalid_argument);

        // Empty parts
        EXPECT_THROW(Version::parse(""), std::invalid_argument);
        EXPECT_THROW(Version::parse("  "), std::invalid_argument);
        EXPECT_THROW(Version::parse("!2.2"), std::invalid_argument);
        EXPECT_THROW(Version::parse("0!"), std::invalid_argument);
        EXPECT_THROW(Version::parse("!"), std::invalid_argument);
        EXPECT_THROW(Version::parse("1."), std::invalid_argument);
        EXPECT_THROW(Version::parse("1..1"), std::invalid_argument);
        EXPECT_THROW(Version::parse("5.5..mw"), std::invalid_argument);
        EXPECT_THROW(Version::parse("1.2post+"), std::invalid_argument);
        EXPECT_THROW(Version::parse("1!+1.1"), std::invalid_argument);

        // Repeated delimiters
        EXPECT_THROW(Version::parse("5.5++"), std::invalid_argument);
        EXPECT_THROW(Version::parse("5.5+1+0.0"), std::invalid_argument);
        EXPECT_THROW(Version::parse("1!2!3.0"), std::invalid_argument);

        // '-' and '_' delimiters not allowed together.
        EXPECT_THROW(Version::parse("1-1_1"), std::invalid_argument);

        // Forbidden characters
        EXPECT_THROW(Version::parse("3.5&1"), std::invalid_argument);
        EXPECT_THROW(Version::parse("3.5|1"), std::invalid_argument);
    }
    ...
    TEST(version, parse_openssl)
    {
        ...
        // Strict ordering
        EXPECT_TRUE(std::is_sorted(versions.cbegin(), versions.cend()));
        // None compare equal (given the is_sorted assumption)
        EXPECT_EQ(std::adjacent_find(versions.cbegin(), versions.cend()), versions.cend());
    }
