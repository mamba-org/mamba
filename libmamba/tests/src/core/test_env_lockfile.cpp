...
namespace mamba
{
    namespace
    {
        ...
        TEST_CASE("env-lockfile trusted conda lockfile depends not in defaulted_keys")
        {
            // Verify that when a conda lockfile entry has sha256 (trustworthy metadata)
            // and provides per-package dependencies, "depends" is NOT listed in
            // defaulted_keys. The lockfile's dependency data may reflect repodata patches
            // and should be treated as authoritative — not erased by write_repodata_record().
            //
