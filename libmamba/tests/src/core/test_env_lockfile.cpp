...
namespace mamba
{
    namespace
    {
        ...
        TEST_CASE("env-lockfile trusted conda lockfile depends/constrains not in defaulted_keys")
        {
            // Verify that when a conda lockfile entry has sha256 (trustworthy metadata)
            // and provides per-package dependencies/constrains, "depends"/"constrains"
            // are NOT listed in defaulted_keys. The lockfile's data may reflect repodata
            // patches and should be treated as authoritative.
            //
            // The sha256 trust indicator distinguishes lockfiles generated by a healthy
            // tool from those generated during v2.1.1 through v2.3.2 (partially mitigated for
            // depends/constrains in v2.3.3 by #4071) when metadata was corrupted.
            //
            // Test data: good_one_package-lock.yaml contains a package (libzlib) with
            //   sha256: 5b7e002932c0138d78d251caae0c571d13f857ff90e7ce21d58d67073381250e
            //   dependencies: {vc: ">=14.1,<15.0a0", vs2015_runtime: ">=14.16.27012"}
            //   constrains: {zlib: "==1.2.11 *_1013"}
            //
            ...
            for (const auto& pkg : packages)
            {
                ...
                // Precondition: this lockfile entry has sha256 (trusted)
                ...
                if (!pkg.info.constrains.empty())
                {
                    CHECK_FALSE(contains(pkg.info.defaulted_keys, "constrains"));
                }
            }
        }
