...
TEST_CASE("MatchSpec parse")
{
    ...
    {
        auto ms = MatchSpec::parse("importlib-metadata  # drop this when dropping Python 3.8").value();
    ...
    SECTION("xtensor[url=file:///home/wolfv/Downloads/xtensor-0.21.4-hc9558a2_0.tar.bz2]")
    {
        ...
        REQUIRE(ms.name().str() == "xtensor");
        REQUIRE(
            ms.channel().value().str() == "file:///home/wolfv/Downloads/xtensor-0.21.4-hc9558a2_0.tar.bz2"
        );
    ...
    SECTION("numpy >1.8,<2|==1.7,!=1.9,~=1.7.1 py34_0")
    {
        ...
        REQUIRE(
            ms.str() == R"ms(numpy[version=">1.8,((<2|==1.7),(!=1.9,(>=1.7.1,=1.7)))",build="py34_0"])ms"
        );
    }
    ...
    {
        auto ms = MatchSpec::parse("conda-canary/linux-64::conda==4.3.21.post699+1dab973=py36h4a561cd_0")
                      .value();
        ...
        REQUIRE(ms.build_string().str() == "py36h4a561cd_0");
        REQUIRE(ms.str() == "conda-canary[linux-64]::conda==4.3.21.0post699+1dab973=py36h4a561cd_0");
    }
...
TEST_CASE("parse_url")
{
    ...
    SECTION(R"(D:\a\mamba\mamba\micromamba\tests\data\cph_test_data-0.0.1-0.tar.bz2)")
    {
        if (util::on_win)
        {
            ...
            REQUIRE(ms.name().str() == "cph_test_data");
            REQUIRE(ms.version().str() == "==0.0.1");
            REQUIRE(ms.str() == "D:/a/mamba/mamba/micromamba/tests/data/cph_test_data-0.0.1-0.tar.bz2");
            REQUIRE(ms.build_string().str() == "0");
            REQUIRE(ms.filename() == "cph_test_data-0.0.1-0.tar.bz2");
        }
    }
