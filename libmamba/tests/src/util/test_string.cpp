...
TEST_CASE("split_prefix")
{
    ...
    REQUIRE(split_prefix("https://localhost", "https://") == PrefixTail{ "https://", "localhost" });
    REQUIRE(split_prefix("https://localhost", "http://") == PrefixTail{ "", "https://localhost" });
...
TEST_CASE("strip_if_parts")
{
    ...
    REQUIRE(strip_if_parts("hello", [](auto) { return false; }) == StrTrio({ "", "hello", "" }));
    REQUIRE(
        strip_if_parts("\n \thello \t\n", [](auto c) { return !is_alphanum(c); })
        == StrTrio({ "\n \t", "hello", " \t\n" })
    );
...
TEST_CASE("concat_dedup_splits")
{
    for (std::string_view sep : { "/", "//", "/////", "./", "./." })
    {
        ...
        REQUIRE(
            concat_dedup_splits(fmt::format("test{}chan-foo", sep), fmt::format("foo{}bar", sep), sep)
