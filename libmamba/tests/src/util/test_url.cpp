...
TEST_CASE("UTL parse")
{
    ...
    SECTION("https://mamba🆒🔬.org/this/is/a/path/?query=123&xyz=3333")
    {
        ...
        const URL url = URL::parse("https://mamba🆒🔬.org/this/is/a/path/?query=123&xyz=3333").value();
...
TEST_CASE("URL pretty_str options")
{
    ...
    SECTION("Credential option")
    {
        ...
        {
            REQUIRE(
                url.pretty_str(URL::StripScheme::no, 0, URL::Credentials::Show) == "https://localhost/"
            );
            REQUIRE(
                url.pretty_str(URL::StripScheme::no, 0, URL::Credentials::Hide) == "https://localhost/"
            );
            REQUIRE(
                url.pretty_str(URL::StripScheme::no, 0, URL::Credentials::Remove) == "https://localhost/"
            );
        }
...
TEST_CASE("str and pretty_str")
{
    SECTION("https://user:password@mamba.org:8080/folder/file.html?param=value#fragment")
    {
        ...
        REQUIRE(url.str() == "https://user:*****@mamba.org:8080/folder/file.html?param=value#fragment");
        REQUIRE(
        ...
        REQUIRE(
            url.pretty_str() == "https://user:*****@mamba.org:8080/folder/file.html?param=value#fragment"
        );
    }
    ...
    SECTION("user@mamba.org")
    {
        ...
        REQUIRE(
            url.pretty_str(URL::StripScheme::yes, '\0', URL::Credentials::Hide) == "user:*****@mamba.org/"
        );
        REQUIRE(
            url.pretty_str(URL::StripScheme::yes, '\0', URL::Credentials::Show) == "user@mamba.org/"
        );
        REQUIRE(url.pretty_str(URL::StripScheme::yes, '\0', URL::Credentials::Remove) == "mamba.org/");
    }
