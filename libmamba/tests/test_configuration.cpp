    ...
    namespace testing
    {
        ...
        TEST_F(Configuration, pkgs_dirs)
        {
            ...
            std::string extra_cache = "\n  - "
                                      + (fs::u8path(env::get("APPDATA").value_or("")) / ".mamba"
                                         / "pkgs")
                                            .string()
                                      + "  # 'fallback'";
#else
...
#define TEST_BOOL_CONFIGURABLE(NAME, CTX)                                                           \
    ...
    {                                                                                               \
        ...
        TEST_F(Configuration, channel_priority)
        {
            ...
            load_test_config({ rc1, rc2, rc3 });
            EXPECT_EQ(config.at("channel_priority").value<ChannelPriority>(), ChannelPriority::kFlexible);
            EXPECT_TRUE(ctx.channel_priority == ChannelPriority::kFlexible);

            load_test_config({ rc3, rc1, rc2 });
            EXPECT_EQ(config.at("channel_priority").value<ChannelPriority>(), ChannelPriority::kDisabled);
        ...
        TEST_F(Configuration, safety_checks)
        {
            ...
            load_test_config({ rc1, rc2, rc3 });
            EXPECT_EQ(config.at("safety_checks").value<VerificationLevel>(), VerificationLevel::kEnabled);
