            ...
            else
            {
                ...
                if (from_history)
                {
                    ...
                    for (const auto& [k, v] : versions_map)
                    {
                        ...
                        {
                            yaml_contents.dependencies.push_back(requested_specs_map[k].to_string());
                            auto chans = channel_context.make_channel(v.channel);
                            for (const auto& chan : chans)
                            {
                ...
                // Handle --no-md5: This option doesn't affect YAML format output
                // (MD5 is only shown in explicit format)
