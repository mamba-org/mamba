...
set_package_command(CLI::App* subcom)
{
    ...
    auto extract_subcom = subcom->add_subcommand("extract", "Extract an archive to a destination folder");
    ...
    auto compress_subcom = subcom->add_subcommand("compress", "Compress a folder to a `.tar.bz2` or a `.conda` file");
    ...
    auto transmute_subcom = subcom->add_subcommand("transmute", "Convert a `.tar.bz2` file to `.conda` a package");
