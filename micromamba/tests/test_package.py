...
def test_extract(cph_test_file: Path, tmp_path: Path):
    ...
    subprocess.call([mamba_exe, "package", "extract", str(tmp_path / "mm" / cph_test_file.name), str(tmp_path / "mm" / "cph_test_data-0.0.1-0")])
...
def test_extract_compress(cph_test_file: Path, tmp_path: Path):
    ...
    mamba_exe = get_umamba()
    out = tmp_path / "mm" / "out"
    subprocess.call([mamba_exe, "package", "extract", str(tmp_path / "mm" / cph_test_file.name), str(out)])
    subprocess.call([mamba_exe, "package", "compress", str(out), str(tmp_path / "mm" / "out.tar.bz2")])
...
def test_transmute(cph_test_file: Path, tmp_path: Path):
    ...
    subprocess.call(["cph", "transmute", str(tmp_path / cph_test_file.name), "--out-folder", str(tmp_path / "cph"), ".conda"])
    ...
    fcmp = filecmp.dircmp(tmp_path / "cph" / "cph_test_data-0.0.1-0", tmp_path / "mm" / "cph_test_data-0.0.1-0")
    print_diff_files(fcmp)
    assert(len(fcmp.left_only) == 0 and len(fcmp.right_only) == 0 and len(fcmp.diff_files) == 0)
    fcmp.report_full_closure()
