...
namespace mamba
{
    namespace testing
    {
        ...
        TEST_F(Configuration, channel_alias)
        {
            ...
            config.at("channel_alias")
                .set_value("https://my.channel")
                .compute_config()
                .set_context();
            EXPECT_EQ(config.dump(true, true),
...
#define EXPECT_CA_EQUAL(CA)                                                                        \
    ...
    {                                                                                              \
        ...
        TEST_F(Configuration, ssl_verify)
        {
            ...
            config.at("ssl_verify").set_value("/new/test").compute_config().set_context();
            EXPECT_EQ(config.dump(true, true),
...
#undef EXPECT_CA_EQUAL
        ...
        TEST_F(Configuration, cacert_path)
        {
            ...
            config.at("cacert_path").set_value("/new/test").compute_config().set_context();
            EXPECT_EQ(config.dump(true, true),
                      unindent((R"(
            ...
            config.at("ssl_verify").compute_config().set_context();
            EXPECT_EQ(config.dump(true, true),
                      unindent((R"(
                                cacert_path: /new/test  # 'API' > 'MAMBA_CACERT_PATH' > ')"
                                + src + R"('
...
#define TEST_BOOL_CONFIGURABLE(NAME, CTX)                                                          \
    ...
    {                                                                                              \
        ...
        config.at(#NAME).set_value("true").compute_config().set_context();                         \
        EXPECT_EQ((config.dump(true, true, false, false, false, false, { #NAME })), expected);     \
        EXPECT_TRUE(config.at(#NAME).value<bool>());                                               \
        EXPECT_TRUE(CTX);                                                                          \
                                                                                                   \
        ...
        TEST_F(Configuration, channel_priority)
        {
            ...
            config.at("channel_priority").set_value("flexible").compute_config().set_context();
            EXPECT_EQ(config.dump(true, true),
        ...
        TEST_F(Configuration, pinned_packages)
        {
            ...
            config.at("pinned_packages").set_value("pytest").compute_config().set_context();
            EXPECT_EQ(config.dump(true, true),
                      unindent((R"(
                                pinned_packages:
                                  - pytest  # 'API'
        ...
        TEST_F(Configuration, safety_checks)
        {
            ...
            config.at("safety_checks").set_value("disabled").compute_config().set_context();
            EXPECT_EQ(config.dump(true, true),
